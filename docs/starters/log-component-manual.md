# Aiko Boot Starter Log 组件使用说明书

## 概述

`@ai-partner-x/aiko-boot-starter-log` 是一个基于 Winston 的简化日志组件，提供类似 SLF4J 风格的 API，支持装饰器语法，易于集成到 TypeScript/JavaScript 项目中。
...
### 核心特性
1. **SLF4J 风格 API**: 提供类似 Java SLF4J 的简洁接口
2. **装饰器支持**: 支持 `@Slf4j` 和 `@Log` 装饰器，类似 Lombok
3. **多传输支持**: 控制台、文件、流等多种日志输出方式
4. **自动配置**: 支持环境变量、配置文件、package.json 自动加载
5. **类型安全**: 完整的 TypeScript 类型定义
6. **高性能**: 基于 Winston 3.x，性能优化
7. **测试完备**: 超过 85% 的测试覆盖率
...
│   └── auto-config-example.ts          # 自动配置示例
├── package.json                        # 包配置
├── tsconfig.json                       # TypeScript 配置
├── tsup.config.ts                      # 构建配置
├── vitest.config.ts                    # 测试配置
├── README.md                           # 项目说明
└── LogComponentManual.md               # 详细使用手册（本文档）
```
...
本组件通过可选的 peerDependency `@ai-partner-x/aiko-boot` 与 Aiko Boot 框架集成。如果安装了该框架，可以使用以下方式自动加载配置：

```typescript
import { initFromAikoBoot, autoInit } from '@ai-partner-x/aiko-boot-starter-log';

// 方式1：手动从 Aiko Boot 初始化
initFromAikoBoot();

// 方式2：自动检测（优先使用 Aiko Boot 配置）
autoInit({ useAikoBoot: true });
```
...
import { ILogger, getLogger } from '@ai-partner-x/aiko-boot-starter-log';
...
export class CustomLogger implements LoggerService {
  ...
  constructor() {
    this.logger = getLogger('nestjs-app');
  }
  ...
  error(message: string, trace?: string, context?: string) {
    this.logger.error(message, new Error(trace), { context });
  }
...
const app = fastify();
const logger = getLogger('fastify-app');

// 注册日志插件
app.addHook('onRequest', (request, reply, done) => {
  logger.http(`Request: ${request.method} ${request.url}`);
  done();
});

app.addHook('onResponse', (request, reply, done) => {
  logger.http(`Response: ${request.method} ${request.url} - ${reply.statusCode}`);
  done();
});
...
// ❌ 不推荐：总是构建消息
logger.debug(`用户 ${user.id} 执行操作 ${operation}，耗时 ${Date.now() - start}ms`);
...
// ✅ 使用便捷方法
logger.isDebugEnabled?.() && logger.debug(`用户 ${user.id} 执行操作 ${operation}`);
```
...
// ❌ 不推荐：总是序列化大对象
logger.debug('数据对象', { 
  data: JSON.stringify(largeObject), // 昂贵的序列化操作
...
const productionConfig = {
  ...
  format: 'json', // 使用 JSON 格式，性能更好
  transports: [
    {
      type: 'file',
      filename: './logs/app.log',
      level: 'info',
      maxSize: '50m', // 合理设置文件大小
      maxFiles: 30, // 保留 30 天日志
    },
    {
      type: 'console',
      level: 'warn', // 控制台只显示警告和错误
      format: 'simple', // 简单格式
    }
  ]
};
...
```typescript
import { createAsyncLogger } from './custom-async-logger';

// 自定义异步日志记录器
const asyncLogger = createAsyncLogger('async-app', {
  bufferSize: 1000, // 缓冲区大小
  flushInterval: 1000, // 刷新间隔（毫秒）
  dropPolicy: 'oldest', // 缓冲区满时丢弃最旧的消息
});

// 批量记录日志
asyncLogger.info('批量操作开始', { batchId: 'batch-123' });
// ... 业务逻辑
asyncLogger.info('批量操作完成', { batchId: 'batch-123', count: 1000 });
```

#### 5.5 内存使用优化
```typescript
// 避免在日志中存储大对象引用
const logger = getLogger('memory-sensitive');

// ❌ 不推荐：存储大对象引用
const largeData = fetchLargeData();
logger.info('处理数据', { data: largeData }); // largeData 会一直保留在内存中

// ✅ 推荐：只存储必要信息
logger.info('处理数据', { 
  dataSize: largeData.length,
  dataHash: calculateHash(largeData),
  processed: true
});
```
...
const logger = getLogger('secure-app');
...
logger.info('交易记录', sanitizeData(transaction));
```
  ...
  transports: [
    {
      ...
      // 设置文件权限（Linux）
      options: {
        mode: 0o640, // 所有者可读写，组成员可读，其他用户无权限
      }
    },
    {
      type: 'file',
...
class EncryptedLogger {
  ...
  constructor(private logger: any, encryptionKey: string) {
  ...
  info(message: string, meta?: any) {
    const encryptedMeta = meta ? this.encrypt(meta) : undefined;
    this.logger.info(message, { encrypted: encryptedMeta });
  }
}
```
...
class ErrorMonitor {
  ...
  private readonly logger = getLogger('error-monitor');
  
  constructor(private threshold: number = 100, private interval: number = 60000) {}
  
  recordError(error: Error, context?: any) {
    this.errorCount++;
    this.logger.error('记录错误', error, context);
  ...
  private triggerAlert() {
    this.logger.error('错误率超过阈值', null, {
    ...
    // 发送告警通知
    // sendAlert({ type: 'ERROR_RATE_HIGH', count: this.errorCount });
  }
...
class PerformanceMonitor {
  private readonly logger = getLogger('performance');
  ...
  measure<T>(operation: string, fn: () => T): T {
    ...
    try {
      ...
      if (duration > 1000) { // 超过1秒记录警告
        this.logger.warn(`操作 ${operation} 耗时过长`, {
      ...
      const duration = performance.now() - start;
      this.logger.error(`操作 ${operation} 失败`, error as Error, {
        duration: `${duration.toFixed(2)}ms`
      });
      throw error;
    }
  ...
  private recordMetric(operation: string, duration: number) {
    ...
    // 每100次记录一次统计信息
    if (this.metrics.get(operation)!.length % 100 === 0) {
  ...
  private logStats(operation: string) {
    ...
    this.logger.info(`性能统计: ${operation}`, {
...
#### 7.3 日志聚合与可视化
```typescript
// 使用 Elasticsearch + Kibana 进行日志聚合
...
// 关键指标监控
const keyMetrics = {
  errorRate: 'error.count / total.count',
  responseTime: 'duration.p99',
  throughput: 'requests.count / interval',
  availability: 'success.count / total.count'
};
```
